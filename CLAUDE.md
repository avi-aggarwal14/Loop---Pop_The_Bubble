...
1. **Keys/accounts** (the blocker to remaining live runs): ✅ Anthropic key (done — brief engine runs live); ✅ mubit key (done — client aligned + compounding verified live); Supabase project + run **both** migrations; Shopify store access + app credentials/OAuth setup; Google Cloud OAuth client (GA4); `APP_URL`; `CRON_SECRET`; (optional) Vercel Pro for drains. **Shopify decision:** use the direct token path as an integration smoke test; do not spend time building a fake realistic dev store for the demo story.
...
- **2026-06-06** — **Public website sidecar added; explicitly not the main analytics product.** Added `scripts/website-public-brief.ts` + `npm run website:brief` (env: `WEBSITE_URL`, optional `WEBSITE_FOUNDER_ID`). It crawls same-host public pages with `fetchSite()`, extracts a `BusinessProfile` via Claude, builds a `WeeklyData` payload with public-only notes, recalls mubit, generates a Growth Brief, and writes the brief lesson back to mubit. Purpose: onboarding/business-context enrichment and a low-friction fallback while first-party connectors are pending. Limitation: it cannot pull sessions, conversion, revenue, orders, customer journeys, ROAS, or inventory; do not pitch it as analytics. Verified with `npm run typecheck` and `npm test` **25/25**.
- **2026-06-06** — **Product/demo decision: Shopify direct path is a connector smoke test, not the demo story.** User clarified not to spend time making a fake/dev Shopify store look real: if it has no real orders, it only proves API plumbing. The main product remains real user first-party analytics connections (Shopify, GA4, Vercel, etc.) → Claude Growth Brief → mubit memory/outcomes. Use `npm run shopify:brief` with `SHOPIFY_SHOP_DOMAIN` + `SHOPIFY_ACCESS_TOKEN` to validate Shopify API/scopes/data normalization quickly. For a compelling demo, use a real/borrowed merchant dataset if available, otherwise keep the existing seeded 2-week narrative for the story and treat Shopify as “connector works.” Public website intelligence remains a sidecar/onboarding enrichment path, not a replacement for analytics.
