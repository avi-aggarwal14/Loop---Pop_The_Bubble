...
0. **✅ DONE — Shopify per-product depth** (ROADMAP Phase 3b): line items + catalogue/inventory → `deriveProductMetrics` (per-product revenue/units WoW, top sellers, inventory-vs-velocity, dead stock), wired through `collect.ts`, rendered in the brief input, fixtures + `tests/products.test.ts` added. Build/test suite green (32 tests total).
1. **Keys/accounts** (the blocker to remaining live runs): ✅ Anthropic key (done — brief engine runs live); ✅ mubit key (done — client aligned + compounding verified live); ✅ Shopify app credentials + **live OAuth in production**; ✅ GA4 credentials configured in prod (round-trip click still pending); Supabase project + run **both** migrations; `CRON_SECRET` on Vercel; (optional) Vercel Pro for drains. **Shopify decision:** production is the multi-merchant OAuth app path; the direct token path is only an integration smoke test.
2. **Dashboard UI** — **the real founder dashboard now exists** at `/dashboard` (`components/dashboard/FounderDashboard.tsx`): blank/onboarding by default, real OAuth connect cards (`/api/connect/status`), **Ask Synapse** + follow-up thread (`/api/advice` + `/followup`), a "Live from your store" KPI panel (`/api/connect/metrics`), history backfill on connect (`/api/connect/backfill`), this-week's brief from the live store (`/api/brief`), and a `?demo=1` walk-through. A separate **DEMO brief card** lives at `/brief`. **Still to do for the real product:** persist connections/briefs in Supabase (they're in the session cookie today), Supabase Auth (visible login UI), list a founder's stored briefs via `getLatestBriefs`, and wire Done/Skipped → `POST /api/briefs/[id]/action`.
3. **Founder session wiring** — connections currently live in the **signed `syn_connect` session cookie** (per-browser), good enough to connect + see real data without Supabase. Production Shopify/Google connector starts can also resolve `founder_id` from a Supabase server session via `@supabase/ssr` (`lib/supabase/auth.ts`); the `founder_id` query param is local/dev fallback only (`ALLOW_QUERY_FOUNDER_ID=true` or non-production). Remaining app-shell work is the visible login/signup UI + DB persistence of the cookie's tokens (which also unblocks encryption-at-rest).
...
- **Shopify orders are windowed by `processed_at`, not `created_at`** — `processed_at` is what Shopify Analytics dates by and the field back-dated/imported orders carry (`created_at` is the API write time, so a back-dated order would land in the wrong week). `fetchShopifyWeek` falls back to `created_at` when `processed_at` is null.
- **Connections live in a signed session cookie today, not Supabase** — `syn_connect` (`lib/connect/session.ts`), per-browser. So there's no encryption-at-rest and no cross-device persistence yet; that's the main thing Supabase persistence unblocks.
- **The Ask/brief must stay honest about a real store** — for a *connected* founder, never borrow the Coconut & Berry `example.ts` data/memories; return `needsConnect`/`needsData` instead. The example is only for `demo:true` (the recordable demo) or the no-key sample fallback.
...
- **2026-06-07** — **Handoff-doc refresh to match the last ~12h of work + Shopify `processed_at` order windowing (uncommitted).** Two things. **(1) Code:** `lib/shopify/ingest.ts` now windows order pulls by **`processed_at`** instead of `created_at` (adds `processed_at` to the field list + the `?processed_at_min/max` query, and maps `createdAt = processed_at ?? created_at`). Rationale: `processed_at` is the date Shopify Analytics uses and the one back-dated/imported orders carry; `created_at` is the API write time, so a back-dated order would otherwise fall in the wrong week. **Verified:** `npm run typecheck` clean and `npm test` **32/32** with the change in tree. Left **uncommitted** for review (it's the only working-tree change). **(2) Docs:** brought the Engineering Handoff fully current after the "Ask Synapse" + dashboard + session-connect + live-OAuth wave that had only been captured in the Project Log narrative — refreshed §1 (TL;DR), §1a (the ad flow is **five** recordable screens `/ad/1→2→3→4→6`; `/ad/5` redirects to `/ad/6`; `/ad/2` click-to-zoom; added the new `demo/synapse-demo-voiceover-script.md`), §2 (added the on-demand entry points), §3 (new bullets for the Ask advisor + session connect), **§4 file map** (added `lib/advise/*`, `lib/connect/session.ts`, `lib/supabase/auth.ts`, `lib/demo/`, `lib/brief/sample.ts`, all the new `app/api/*` routes — advice/brief/connect/webhooks/demo — and `app/dashboard`, `app/connect`, `app/demo/shopify`, `app/ad/[step]`, `components/dashboard/FounderDashboard`, `components/connect/ConnectSources`, plus the new scripts), §6 (new env vars: `SESSION_SECRET`, direct-token smoke vars, `ASK_FOUNDER_ID`, `ALLOW_QUERY_FOUNDER_ID`, sheets/drain vars), §7 (new npm scripts, deps `@supabase/ssr`/`@shopify/cli`), §8/§9 (Shopify OAuth verified live; GA4 configured; dashboard now real) and §10 (new gotchas). **Normalised the stale `28/28` test count to `32/32` throughout** (CLAUDE.md, README, ROADMAP). No runtime code beyond the `ingest.ts` change.
