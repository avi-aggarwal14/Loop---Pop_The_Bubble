...
1. **Keys/accounts** (the blocker to remaining live runs): ✅ Anthropic key (done — brief engine runs live); ✅ mubit key (done — client aligned + compounding verified live); Supabase project + run **both** migrations; Shopify Partners app + dev store; Google Cloud OAuth client (GA4); `APP_URL`; `CRON_SECRET`; (optional) Vercel Pro for drains.
...
- **2026-06-06** — **Two core product capabilities written into the brief: (1) instant historical backfill, (2) the ask-anytime advisor.** Until now the docs only described the *pushed weekly brief*. Added to "What Synapse Is": **(1) Instant history** — on connect, pull the founder's whole back-catalogue (months/years of Shopify orders + traffic) into mubit so the *first* brief is already deeply informed (no cold start). **(2) Ask-anytime advisor** — the founder can ask Synapse about a specific decision ("should I discount X?") and it answers from memory (full analytics history + past briefs + outcomes). Both lean on the already-live memory layer. **Immediate build implications (the next work):** (a) **Shopify data pulling** — extend the Shopify pull from last-week-vs-prior to a **historical backfill** (N months → per-week summaries → ingest each into mubit); a live single-store pull path already exists (`npm run shopify:brief`, see entry below). (b) **brief generation / growth advice** — generate the *first* brief off the backfilled history (prove "instant smart"), and build the **ask path**: an `askAdvice(question)` engine fn (recall memory → Claude answers the question, same recall→generate shape as the brief) + a route + a small ask box on `/brief`. Neither reinvents anything — both extend the proven memory engine. Docs only this pass; no code yet.
