...
- ✅ **Verified:** `npm run typecheck` clean; `npm test` = **25/25** pass (incl. `tests/brief.test.ts` — the engine via a mocked Anthropic client); **a real Anthropic Claude generation** — `npm run generate-brief` produces schema-valid Growth Briefs end-to-end (structured output + Zod parse), with prompt caching hitting on the 2nd call.
